#ifndef __ONE_ORAM_BTREE_H__
#define __ONE_ORAM_BTREE_H__
...
class OneORAMBTree {
public:
    OneORAMBTree(const uint32_t mode, const std::string scale, const uint32_t table_id, const Schema* in_sch, const uint32_t o_height, std::unordered_map <uint32_t, std::string>& blocks, uint32_t& block_id, const std::string& in_prefix) {
        // Load Meta-Data
        sch = in_sch;
        btree_sch = in_sch;
    ...
    OneORAMBTree(const Schema* in_sch, const std::string& in_prefix) {
        sch = in_sch;
        btree_sch = in_sch;
    ...
    void getNextTuple(bool& next, uint32_t attrID, char* leaf, int32_t& leafIndex, char* data, int32_t& dataBID, int32_t& dataIndex) {
        ...
        if (leafIndex >= entryNum) {
            ...
            if (next_leaf_id >= 0) {
                std::string value;
                if (outsourced_height[h] > 0) value = one_oram->get(next_leaf_id);
                ...
                if (outsourced_height[h] > 0) getDummyBlock();
                getDummyBlock();
                leafIndex = -1; dataBID = -1; dataIndex = -1;
                return;
            }
