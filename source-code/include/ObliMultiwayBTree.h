#ifndef __OBLI_MULTIWAY_BTREE_H__
#define __OBLI_MULTIWAY_BTREE_H__
...
template<class T>
class ObliMultiwayBTree {
public:
    ObliMultiwayBTree(const uint32_t mode, const std::string scale, const uint32_t table_id, const Schema* in_sch, const uint32_t o_height, const std::string& in_dst, const std::string& in_prefix) {
        // Load Meta-Data
        sch = in_sch;
        btree_sch = in_sch;
    ...
    ObliMultiwayBTree(const Schema* in_sch, const std::string& in_prefix) {
        sch = in_sch;
        btree_sch = in_sch;
    ...
    bool disableCurrentTuple(uint32_t attr_id, char** all_block, int32_t* all_bid, int32_t* all_index, char* pre_leaf, int32_t& pre_leaf_bid) {
        ...
        for (int32_t i = 0; i < height[h]; ++i) {
            ...
            else index_oram[h]->put(all_bid[i], value);
        }
        getDummyDataBlock();
        return updated;
    }
    ...
    void updatePreLeaf(uint32_t attr_id, char* pre_leaf, int32_t& pre_leaf_bid) {
        ...
        for (int32_t i = 0; i < height[h]; ++i) {
            if (i == height[h] - 1 && pre_leaf_bid >= 0) {
                ...
                else index_oram[h]->put(pre_leaf_bid, value);
            }
            else if (i >= cached_height[h]) getDummyIndexBlock(attr_id);
        }
        getDummyDataBlock();
    }
    ...
    void resetFlags(uint32_t attr_id) {
        ...
        for (int32_t i = 0; i < height[h] - 1; ++i) {
            ...
            for (int32_t now = lBlock; now <= rBlock; ++now) {
                ...
                else index_oram[h]->put(now, value);
            }
            lBlock = nextL;
            rBlock = nextR;
        }
        ...
        for (int32_t now = lBlock; now <= rBlock; ++now) {
            ...
            for (int32_t j = 0; j < tmp_cnt; ++j) {
                ...
                if (j < tmp_cnt - 1)
                    ...
                    else nextValue = index_oram[h]->get(now + 1);
...
#endif //__OBLI_MULTIWAY_BTREE_H__
