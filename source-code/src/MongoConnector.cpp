...
MongoConnector::MongoConnector(const std::string& url, const std::string& collection_name, const bool create)
...
void MongoConnector::resize(const uint32_t& len, const std::string& ns) {
    std::string mongo_collection = (ns == "") ? collection_name : ns;
    char buf[100];
...
void MongoConnector::insert(const char* blocks, const uint32_t& low, const uint32_t& high, const std::string& ns) {
    std::string mongo_collection = (ns == "") ? collection_name : ns;
    std::vector<BSONObj> objs;
    BinDataType bin_type = BinDataGeneral;
    ...
    for (uint32_t i = low; i <= high; ++i) {
        objs.push_back(BSONObjBuilder().append("id", i)
            .appendBinData("data", (int)B, bin_type,
...
std::string MongoConnector::fetch(const uint32_t& id, const std::string& ns) {
    std::string mongo_collection = (ns == "") ? collection_name : ns;
...
void MongoConnector::fetch(const uint32_t& low, const uint32_t& high, char* blocks, const std::string& ns) {
    std::string mongo_collection = (ns == "") ? collection_name : ns;
    char buf[100];
...
void MongoConnector::find(const uint32_t& low, const uint32_t& high, char* blocks, const std::string& ns) {
    std::string mongo_collection = (ns == "") ? collection_name : ns;
    char buf[100];
    sprintf(buf, "{id: {$gte: %d, $lte: %d}}", low, high);
    std::unique_ptr<DBClientCursor> cursor = mongo.query(mongo_collection, Query(buf).sort("id"));
...
void MongoConnector::update(const char* blocks, const uint32_t& low, const uint32_t& high, const std::string& ns) {
    std::string mongo_collection = (ns == "") ? collection_name : ns;
    char buf[100];
    sprintf(buf, "{id: {$gte: %d, $lte: %d}}", low, high);
    mongo.remove(mongo_collection, Query(buf));
    insert(blocks, low, high, ns);
}
...
void MongoConnector::remove(const uint32_t& id, const std::string& ns) {
    std::string mongo_collection = (ns == "") ? collection_name : ns;
    mongo.remove(mongo_collection, BSON("id" << id));
}

void MongoConnector::remove(const std::string& id, const std::string& ns) {
    std::string mongo_collection = (ns == "") ? collection_name : ns;
    mongo.remove(mongo_collection, BSON("id" << id));
}

void MongoConnector::remove(const uint32_t& low, const uint32_t& high, const std::string& ns) {
    std::string mongo_collection = (ns == "") ? collection_name : ns;
    char buf[100];
    sprintf(buf, "{id: {$gte: %d, $lte: %d}}", low, high);
    mongo.remove(mongo_collection, Query(buf));
