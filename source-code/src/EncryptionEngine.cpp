...
int encryption_engine::encrypt(const unsigned char *plaintext, int plaintext_len, const unsigned char *key,
    unsigned char *ciphertext) {
    EVP_CIPHER_CTX *ctx;
    int len;
    int ciphertext_len;

    std::string iv_string = generate_random_block(aes_block_size);
    unsigned char* iv = (unsigned char *)iv_string.c_str();
...
int encryption_engine::decrypt(const unsigned char *ciphertext, int ciphertext_len, const unsigned char *key,
    unsigned char *plaintext) {
    EVP_CIPHER_CTX *ctx;
    int len;
    int plaintext_len;

    const unsigned char* iv = ciphertext + ciphertext_len;
...
void encryption_engine::encrypt(const std::string &plain_text, const byte* enc_key, std::string &cipher_text) {
    unsigned char cipher_text_[B];
    int text_len = encrypt((unsigned char *)plain_text.c_str(), plain_text.length(), enc_key, cipher_text_);
    assert(text_len > 0);
    cipher_text = std::string((const char *)cipher_text_, text_len);
};

void encryption_engine::decrypt(const std::string &cipher_text, const byte* enc_key, std::string &plain_text) {
    unsigned char text[B];
    int text_len = decrypt((unsigned char *)cipher_text.c_str(), cipher_text.length() - aes_block_size, enc_key, text);
    assert(text_len > 0);
    //TODO:
    //plain_text = std::string(text, std::find(text, text + text_len, '\0'));
    plain_text = std::string((const char *)text, text_len);
};
