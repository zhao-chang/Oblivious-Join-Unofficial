...
int main(int argc, char* argv[]) {
    if (argc != 5) {
        printf("Usage: ./one_type1_multi_join <mode> <scale> <outsourced_height> <query_file>\n");
        return -1;
    }
    
    const uint32_t mode = atoi(argv[1]);
    const std::string method = "1mj";
    ...
    OneORAMMultiwayJoin <PathORAM>* oij = new OneORAMMultiwayJoin <PathORAM> (mode, meta_prefix);
    ...
    oij->ObliBNLJ(n_tables, table_id, parent_id, attr_id, parent_attr_id, n_proj_cols, proj_id);
